...
# working directory
WD=/home/viniciuscarvalho/220418-renan/samsa2
cd $WD
...
####################################################################
#
# STEP 1: CLEANING FILES WITH TRIMMOMATIC ## PASS
...
####################################################################
#
# STEP 2: MERGING OF PAIRED-END FILES USING PEAR # PASS
...
####################################################################
#
# STEP 2.9: GETTING RAW SEQUENCES COUNTS # PASS
...
####################################################################
#
# STEP 3: REMOVING RIBOSOMAL READS WITH SORTMERNA # PASS
...
# BEFORE: for file in $STEP_4/*RefSeq_annotated
# AFTER:for file in $STEP_4/*RefSeq_annot_referenceIDs*
for file in $STEP_4/*RefSeq_annotated
do
  checked python $PY_DIR/DIAMOND_analysis_counter.py -I $file -D $RefSeq_db -O -R
  checked python $PY_DIR/DIAMOND_analysis_counter.py -I $file -D $RefSeq_db -F -R
done
